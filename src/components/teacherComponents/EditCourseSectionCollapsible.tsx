...
export default function EditCourseSectionCollapsible ({
    section,
    onAddMaterial,
    onDeleteMaterial
}:{
    section:CourseSection;
    onAddMaterial: (sectionId: number) =>void;
    onDeleteMaterial:(sectionId:number,materialId:number)=>void;
}){
    ...
    return(
        <View className="flex-col gap-1.5 mt-2">
            <TouchableOpacity
                ...
                activeOpacity={0.8}
                className="flex-row items-center justify-between w-full bg-bg-2 border border-border rounded-xl p-4 shadow-md"
            >
            ...
            <Collapsible collapsed={!openSection}>
                <View className="bg-bg-1 border border-border rounded-xl p-3 flex-col gap-2">
                        ...
                        section.materials.map((material, idx) => (
                            <View key={idx} className="flex-row items-center justify-between bg-bg-2 border border-border rounded-lg p-3">
                                ...
                                <TouchableOpacity 
                                    ...
                                    onPress={()=>section.id && material.id && onDeleteMaterial(section.id,material.id)}
                                >
                    ...
                    <TouchableOpacity
                        onPress={() => section.id && onAddMaterial(section?.id)}
